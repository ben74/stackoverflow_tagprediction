...
# avec les mêmes paramètres devant retourner le même nombre de dimensions
# ? => retourne matrice vide => bof bof
    if pca2:
        MBW2 = pca2.fit_transform(MBW)
        sumVariance = round(
            list(
                pca2.explained_variance_ratio_.cumsum()).pop(),
            2)
        p('NbDimensions:', MBW.shape[1], ', sum variance:', sumVariance)
    else:
        MBW = np.array(MBW)
# assert(len(MBW[0]),'/',len(bag_train[0]))#is 3000 / 3000  => fits
# ValueError: X has 3000 features per sample; expecting 1011 ( nb
# dimensions de réduction pca )
...
# relatifs au modèle :: ValueError: Expected indicator for 40 classes, but
# got 358
nb = 1
...
# ValueError: Expected indicator for 358 classes, but got 345 ==> WHY ???
# wasn't re-trained at all  <<< just loaded
...
def predict():
    if (request.method == 'POST'):
        ...
        newdf = pd.DataFrame({'title': [inputCleaner(title)], 'body': [
                             inputCleaner(body)], 'tags': ''})
        tags1, tags2 = EvaluateBagModel(
            globals()[mdlname], newdf, g(
                'bagOfWords_' + k3))
        tags1 = ' , '.join(tags1)
        tags2 = ' , '.join(tags2)
        ldatags = ' , '.join(
